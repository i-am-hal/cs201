...
bool sequentialSearch(vector<int> keys, vector<string> values, int thekey, string &thevalue) {
    for(int i = 0; i < keys.size(); i++) {
        int currentKey = keys.at(i); //Currently looked at key

        //If we have found the current key, then set thevalue to the
        // corresponding value and return true.
        if (currentKey == thekey) {
            thevalue = values.at(i);
            return true;
        }
    }
    return false; //Key doesn't exist within this dataset
}
